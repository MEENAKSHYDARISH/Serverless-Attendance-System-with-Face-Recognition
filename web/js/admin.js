...
function renderSummary(items) {
  ...
  document.getElementById('summary').textContent = `Total: ${summary.total} | Present: ${summary.present} | Late: ${summary.late} | Absent: ${summary.absent}`;
}
...
async function loadRecords() {
  ...
  const query = buildQuery({ employee_id: employeeId, date_from: dateFrom, date_to: dateTo, status });
  const res = await fetch(`${window.APP_CONFIG.API_BASE_URL}/admin/attendance?${query}`, {
