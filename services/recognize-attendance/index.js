  ...
  await ddb.send(
    new PutCommand({
      ...
      Item: {
        upload_id: uploadId,
        ...payload,
        expires_at: toEpochSecondsPlus(24),
      },
    }),
  );
}
...
exports.handler = async (event) => {
  for (const record of event.Records || []) {
    ...
    const uploadId =
      key.split("/").pop()?.replace(".jpg", "") || `unknown-${Date.now()}`;
    ...
    try {
      ...
      if (
        !topMatch ||
        Number(topMatch.Similarity || 0) <
          Number(process.env.MATCH_THRESHOLD || "90")
      ) {
        await putResult(uploadId, {
          state: "COMPLETED",
          action: "none",
          status: "UNRECOGNIZED",
      ...
      if (!employeeId) {
        await putResult(uploadId, {
          ...
          action: "none",
          status: "ERROR",
          message: "Matched face has no employee mapping",
          processed_at: new Date().toISOString(),
        });
        continue;
      }

      const { date, time, iso } = localDateParts(
        process.env.TZ || "Asia/Kolkata",
      );

      const employeeResp = await ddb.send(
        new GetCommand({
      ...
      if (!existing) {
        ...
        await putResult(uploadId, {
          ...
          action: "CLOCK_IN",
          status,
          matched_confidence: Number(confidence),
          processed_at: iso,
        });
        continue;
      }
