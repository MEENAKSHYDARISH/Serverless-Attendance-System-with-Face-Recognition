...
exports.handler = async () => {
  ...
  for (const item of items) {
    byEmployee[item.employee_id] = byEmployee[item.employee_id] || { PRESENT: 0, LATE: 0, ABSENT: 0, total: 0 };
    byEmployee[item.employee_id][item.status] = (byEmployee[item.employee_id][item.status] || 0) + 1;
  ...
  const summary = {
    ...
    week_end: endDate,
    org_present_pct: totals.total ? Number(((totals.present / totals.total) * 100).toFixed(2)) : 0,
    org_late_pct: totals.total ? Number(((totals.late / totals.total) * 100).toFixed(2)) : 0,
    org_absent_pct: totals.total ? Number(((totals.absent / totals.total) * 100).toFixed(2)) : 0,
    totals,
    employees,
  };
